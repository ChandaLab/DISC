...
for ii = idx'
    mu(1) = data.rois(ii, gui.channelIdx).disc_fit.components(1,2);
    st(1) = data.rois(ii, gui.channelIdx).disc_fit.components(1,3);
    for jj = 2:length(data.rois(ii, gui.channelIdx).disc_fit.components(:,2))
        mu(jj) = data.rois(ii, gui.channelIdx).disc_fit.components(jj,2);
        st(jj) = data.rois(ii, gui.channelIdx).disc_fit.components(jj,3);
        snr_comp(jj-1) = (mu(jj) - mu(jj-1))/st(jj-1);
    end
    % set SNR to NaN if only one component is found
    if size(data.rois(ii, gui.channelIdx).disc_fit.components, 1) == 1
        data.rois(ii, gui.channelIdx).SNR = NaN;
    else
        data.rois(ii, gui.channelIdx).SNR = mean(snr_comp);
    end
end
...
if histplot
    figure();
    histogram(vertcat(data.rois(:, gui.channelIdx).SNR));
    title('SNR Histogram')
    xlabel('SNR')
    ylabel('Count')
end
